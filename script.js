'use strict';
// ═══════════════════════════════════════════════════════════════
// SPENDLY AI v3.0 — COMPLETE JS ENGINE
// Author: Yasir Shaikh | GitHub: https://github.com/YasirShaikh03
// Enhanced & completed with Claude AI integration
// ═══════════════════════════════════════════════════════════════
...
const WALLET_COLORS={cash:'linear-gradient(135deg,#16a34a,#15803d)',bank:'linear-gradient(135deg,#1d4ed8,#1e40af)',upi:'linear-gradient(135deg,#7c3aed,#6d28d9)',crypto:'linear-gradient(135deg,#b45309,#92400e)',savings:'linear-gradient(135deg,#0f766e,#0d9488)',credit:'linear-gradient(135deg,#be123c,#9f1239)'};
...
function pinPress(val){
  ...
  updatePinDots();
  if(S.pinBuffer.length===4){if(S.pinBuffer===S.pin){setTimeout(()=>{launchApp();S.pinBuffer='';updatePinDots();},200);}else{setTimeout(showPinError,200);}}
}
...
function togglePin(){
  ...
  else{const p=prompt('Set a 4-digit PIN (blank to cancel):');if(p&&/^\d{4}$/.test(p)){S.pin=p;localStorage.setItem('spendly_pin',p);showToast('PIN lock enabled 🔒');}else if(p)showToast('PIN must be exactly 4 digits');}
}
...
function computeHealthScore(){
  ...
  const spent={};monthTxs.filter(t=>t.type==='expense').forEach(t=>spent[t.category]=(spent[t.category]||0)+t.amount);
...
function refreshDigest(){
  ...
  const cats={};monthTxs.filter(t=>t.type==='expense').forEach(t=>cats[t.category]=(cats[t.category]||0)+t.amount);
...
function renderSubscriptionMini(){
  const subs=detectSubscriptions();const el=document.getElementById('subList');const count=document.getElementById('subCount');
...
function renderHBarChart(){
  ...
  const cats={};getMonthTxs().filter(t=>t.type==='expense').forEach(t=>cats[t.category]=(cats[t.category]||0)+t.amount);
...
function renderStatsTable(){
  ...
  const cats={};const monthTxs=getMonthTxs();monthTxs.filter(t=>t.type==='expense').forEach(t=>cats[t.category]=(cats[t.category]||0)+t.amount);
...
function renderHeatmap(){
  ...
  for(let w=11;w>=0;w--){html+='<div class="heatmap-week">';for(let d=0;d<7;d++){const date=new Date(today);date.setDate(today.getDate()-(w*7+d));const ds=date.toISOString().split('T')[0];const spend=dailySpend[ds]||0;const ins=spend/maxSpend;const cls=ins>0.7?'extreme':ins>0.4?'high':ins>0.15?'med':ins>0?'low':'';html+=`<div class="heatmap-day${cls?' '+cls:''}" title="${ds}: ${fmt(spend)}"></div>`;}html+='</div>';}
...
function renderCalendar(){
  ...
  for(let d=1;d<=daysInMonth;d++){const isToday=today.getDate()===d&&today.getMonth()===month&&today.getFullYear()===year;const hasEvent=eventDays.has(d);html+=`<div class="cal-day${isToday?' today':''}${hasEvent?' has-event':''}">${d}</div>`;}
